...
export default function Configuracion() {
    ...
    useEffect(() => {
        ...
        if (flashData?.obsConfigData) {
            setObsHost(flashData.obsConfigData.host);
            setObsPort(flashData.obsConfigData.port);
            setHasObsConfig(true);
            setObsData({
                host: flashData.obsConfigData.host,
                port: flashData.obsConfigData.port,
                password: ''
            });
        }
    ...
    const handleObsSubmit = (e: React.FormEvent) => {
        ...
        postObs('/api/obs-config', {
            preserveScroll: true,
            onSuccess: () => {
                // Limpiar la contraseña después de guardar exitosamente
                setObsPassword('');
                setObsData('password', '');
            },
            onError: (errors) => {
                ...
                if (errors.host) {
                    toast.error(`Host: ${errors.host}`);
                } else if (errors.port) {
                    toast.error(`Puerto: ${errors.port}`);
                } else if (errors.password) {
                    toast.error(`Contraseña: ${errors.password}`);
            ...
            onFinish: () => {
                // Mostrar mensaje temporal de guardado independientemente del resultado
                toast.success('Configuración OBS procesada correctamente');
            }
        });
    };
    ...
    const deleteObsConfig = () => {
        router.delete('/api/obs-config', {
            preserveScroll: true,
            onSuccess: () => {
                // El mensaje de éxito se maneja en el useEffect con props.flash
                setHasObsConfig(false);
                        ...
                        <CardContent className="space-y-4">
                                        ...
                                        <div className="space-y-3">
                                            <div>
                                                <Label className="text-sm font-medium text-blue-800 dark:text-blue-200">
                                                    Para visualizar el contenido de Retransmisión (Debe ir como fuente de navegador en OBS):
                                                </Label>
                                            ...
                                            <div>
                                                ...
                                                <div className="mt-1 space-y-2">
                                                    <div>
                                                        ...
                                                        <div className="flex gap-2">
                                                            ...
                                                            <Button
                                                                variant="outline"
                                                                size="icon"
                                                                onClick={() => copyToClipboard("rtmps://live.repo.net.pe/live/")}
                                                    ...
                                                    <div>
                                                        <Label className="text-xs text-blue-700 dark:text-blue-300">Clave de transmisión:</Label>
                                                        <div className="flex gap-2">
                                                            <Input
                                                                type="text"
                                                                value={streamKey}
                            ...
                            <div className="mt-4 p-4 bg-muted rounded-lg">
                                ...
                                <ul className="text-sm text-muted-foreground space-y-1">
                                    <li>• <strong>OBS Studio:</strong> Ve a un canal de transmisión</li>
                                    <li>• <strong>Añade un fuente navegador y coloca la URL: </strong> {`https://streamff.repo.net.pe/live?key=${streamKey}`} </li>
                                    <li>• <strong>En tu dispositivo de donde retransmitiras coloca la URL y la clave</strong> </li>
